...
export async function getNetworkDrivesMounted(set_data: Dispatch<SetStateAction<string[]>>) {
    exec("/sbin/mount", (_err, stdout: string) => {
        const mounted_networkDrives_strline = stdout.split("\n").filter((line: string) => { return line.includes(get_pref_smb_ip())&&line.includes(get_pref_smb_usr()); });
        const mounted_networkDrives = mounted_networkDrives_strline.map((item:string) => {return item.split("on /Volumes/")[1].split(" (")[0]});
